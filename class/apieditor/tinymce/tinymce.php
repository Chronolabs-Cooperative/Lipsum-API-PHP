...
class TinyMCE
{
    ...
    public function __construct($config)
    {
        ...
        $this->apiPlugins            = $this->get_apiPlugins();
    ...
    public function init()
    {
        ...
        if (isset($this->config['language']) && is_readable(API_ROOT_PATH . $this->rootpath . '/langs/' . $this->config['language'] . '.js')) {
        ...
        if (!empty($this->config['theme']) && is_dir(API_ROOT_PATH . $this->rootpath . '/themes/' . $this->config['theme'])) {
            ...
            for ($i = 1; $i <= 4; ++$i) {
                ...
                if (isset($this->setting['theme_' . $this->setting['theme'] . "_buttons{$i}"])) {
                    ...
                    foreach ($checklist as $plugin) {
                        if (strpos(strtolower($plugin), 'api') != false) {
                            if (in_array($plugin, $this->apiPlugins)) {
                                $buttons[] = $plugin;
                            }
                        } else {
                            $buttons[] = $plugin;
        ...
        if (!is_dir(API_ROOT_PATH . $this->rootpath . '/themes/' . $this->setting['theme'] . '/docs/' . $this->setting['language'] . '/')) {
    ...
    {
        $plugins      = array();
        $plugins_list = APILists::getDirListAsArray(API_ROOT_PATH . $this->rootpath . '/plugins');
    ...
    // return all api plugins
    /**
     * @return array
     */
    public function get_apiPlugins()
    {
        $apiPlugins = array();
        $allplugins   = APILists::getDirListAsArray(API_ROOT_PATH . $this->rootpath . '/plugins');
        foreach ($allplugins as $plugin) {
            if (strpos(strtolower($plugin), 'api') != false && file_exists(API_ROOT_PATH . $this->config['rootpath'] . "/include/$plugin.php")) {
                if ($right = @include API_ROOT_PATH . $this->config['rootpath'] . "/include/$plugin.php") {
                    $apiPlugins[$plugin] = $plugin;
                }
            }
        }
    ...
    public function render()
    {
        ...
        if (!empty($this->setting['file_browser_callback'])) {
            $fbc_name = API_ROOT_PATH . '/class/apieditor/tinymce/include/' . $this->setting['file_browser_callback'] . '.js';
            //suis passé la /lesrevespossibles/x244/class/apieditor/tinymce/tinymce/jscripts/include/openFinder.js
            $file_browser_callback = "MyAPIUrl ='" . API_URL . "';\n";
        ...
        */
        ...
        if ($isTinyMceJsLoaded) {
            ...
            $ret .= "<script type='text/javascript' src='" . API_URL . $this->rootpath . "/tiny_mce.js'></script>\n";
